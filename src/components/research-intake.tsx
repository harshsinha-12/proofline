  ...
  return (
    <form onSubmit={onSubmit} className="mt-8 space-y-5">
      <div className="space-y-2">
        ...
        <Input id="linkedin" type="url" maxLength={2000} value={linkedInUrl} onChange={(event) => setLinkedInUrl(event.target.value)} required />
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="name">Name hint</Label>
          <Input id="name" maxLength={150} value={nameHint} onChange={(event) => setNameHint(event.target.value)} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="company">Company hint</Label>
          <Input id="company" maxLength={150} value={companyHint} onChange={(event) => setCompanyHint(event.target.value)} />
        </div>
      </div>
