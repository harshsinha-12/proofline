...
export async function verifyPassOne(context: PipelineContext): Promise<StageResult> {
  const claim = (await getClaims(context.run.id)).find((claim) => !claim.check1);
  ...
  if (!source) {
    await saveClaim(context.run.id, { ...claim, check1: emptyCheck(claim, 1, "No accessible origin page supports a direct check.") }, context.token);
  ...
  await saveClaim(context.run.id, { ...claim, check1 }, context.token);
  return { key: `check1:${claim.id}` };
}
